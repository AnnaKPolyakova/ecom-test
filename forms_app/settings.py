...
class Settings(BaseSettings):
    ...
    form_collection: str = Field(
        json_schema_extra={"env": "FORM_COLLECTION"},
        default='form'
    )
    mongo_host: str = Field(
        json_schema_extra={"env": "MONGO_HOST"},
        default='localhost'
    )
    mongo_port: int = Field(json_schema_extra={"env": "MONGO_PORT"}, default=27017)
    number_of_test_data: int = Field(json_schema_extra={"env":
                                                            "NUMBER_OF_TEST_DATA"}, default=100)
